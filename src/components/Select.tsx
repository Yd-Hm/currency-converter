...
const Select: React.FC<IProps> = ({ label, value, options, stateChanger }) => {
  ...
  useEffect(() => {
    setSelectedOption(
      options.filter((option) => option.value === value).pop() ?? options[0]
    );

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value]);

  const select = (option: IOptions) => {
    ...
    <div className="relative mt-1">
      ...
      >
        ...
        <div className="flex items-center">
          <img
            src={"flags/" + selectedOption.value + ".png"}
            className="w-8 rounded-sm mr-3"
        ...
        <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center">
          <svg
            className="h-5 w-5 text-gray-400 group-hover:text-indigo-400 group-focus:text-indigo-600"
